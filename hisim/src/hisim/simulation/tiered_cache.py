"""Framework-neutral HBM/DRAM/SSD KV-cache policy model for HiSim.

Framework adapters retain ownership of their native HBM scheduler state and
report local HBM/DRAM hits to this module.  This shared core owns external-tier
capacity, eviction, promotion, persistence, transfer cost, metrics, and the
retrieve-versus-recompute decision used by both vLLM and SGLang.
"""
...
class TieredLookupDecision:
    ...
    selected_tiers: list[str] = field(default_factory=list, repr=False)

    def trace_dict(self) -> dict[str, Any]:
        values = asdict(self)
        values.pop("selected_keys", None)
        values.pop("selected_tiers", None)
        return values
...
class TieredKVCache:
    """Inclusive-HBM, exclusive-DRAM/SSD cache shared by framework adapters."""
    ...
    def contiguous_available_blocks(self, block_keys: Iterable[str]) -> int:
        """Return the consecutive cached prefix length without mutating policy state."""

        count = 0
        for key in block_keys:
            if self.lookup_tier(key) is None:
                break
            count += 1
        return count

    def touch_blocks(self, block_keys: Iterable[str]) -> None:
        """Refresh copies whose local framework tier satisfied the lookup.

        HBM is inclusive in the shared model, so a framework-local HBM/DRAM
        hit can still refresh an external write-through copy for LRU.  FIFO
        deliberately ignores ``last_accessed_at`` when choosing a victim.
        """

        for key in block_keys:
            sequence = self._next_sequence()
            if self.dram.access(key, sequence) is not None:
                continue
    ...
    def plan_lookup(
        ...
        *,
        ...
        self.touch_blocks(all_keys[:first_external_block])
        return self.plan_lookup_by_block_keys(
            request_id=request_id,
            block_keys=all_keys[first_external_block:],
            local_hbm_tokens=local_hbm_tokens,
            local_dram_tokens=0,
            prompt_tokens=len(prompt_token_ids),
        )

    def plan_lookup_by_block_keys(
        self,
        *,
        request_id: str,
        block_keys: Iterable[str],
        local_hbm_tokens: int = 0,
        local_dram_tokens: int = 0,
        prompt_tokens: int | None = None,
        promote_selected: bool = True,
    ) -> TieredLookupDecision:
        """Plan an external lookup from framework-native prefix block keys.

        ``block_keys`` starts immediately after the framework-resident HBM and
        DRAM prefix.  This form lets SGLang reuse its native page hashes while
        vLLM continues to use :meth:`plan_lookup` with raw prompt token ids.
        """

        block_keys = list(block_keys)
        local_hbm_tokens = max(
            0, local_hbm_tokens // self.block_size * self.block_size
        )
        local_dram_tokens = max(
            0, local_dram_tokens // self.block_size * self.block_size
        )
        local_resident_tokens = local_hbm_tokens + local_dram_tokens
        if prompt_tokens is None:
            prompt_tokens = (
                local_resident_tokens + len(block_keys) * self.block_size + 1
            )
        prompt_tokens = max(1, int(prompt_tokens))
        max_cacheable_tokens = (
            max(0, prompt_tokens - 1) // self.block_size * self.block_size
        )
        local_hbm_tokens = min(local_hbm_tokens, max_cacheable_tokens)
        local_dram_tokens = min(
            local_dram_tokens, max_cacheable_tokens - local_hbm_tokens
        )
        local_resident_tokens = local_hbm_tokens + local_dram_tokens
        max_external_blocks = max(
            0,
            (max_cacheable_tokens - local_resident_tokens) // self.block_size,
        )
        candidates: list[tuple[str, str]] = []
        for key in block_keys[:max_external_blocks]:
        ...
        baseline_tokens = max(1, prompt_tokens - local_resident_tokens)
        baseline_compute_ms = self._recompute_latency_ms(
            start_token=local_resident_tokens,
        ...
        selected_compute_ms = self._recompute_latency_ms(
            start_token=local_resident_tokens + selected_tokens,
            end_token=prompt_tokens,
        )
        ...
        decision = TieredLookupDecision(
            ...
            selected_tiers=[tier for _, tier in selected],
        )
        self._apply_lookup(
            decision,
            selected,
    ...
    def _apply_lookup(
        ...
        *,
        ...
        self.metrics.dram_hit_tokens += decision.local_dram_tokens
        ...
        if promote_selected:
            self._promote_selected(selected)

    def commit_selected_prefix(
        self,
        decision: TieredLookupDecision,
        *,
        start_block: int,
        ...
        """Commit the part of a deferred lookup that actually completed.

        Frameworks with best-effort asynchronous prefetch can select a longer
        prefix than they finish loading.  Promotion is therefore deferred
        until the adapter reports completed pages.
        """

        start = max(0, int(start_block))
        end = min(len(decision.selected_keys), max(start, int(end_block)))
        selected = []
        for key in decision.selected_keys[start:end]:
            tier = self.lookup_tier(key)
            if tier is not None:
                selected.append((key, tier))
        self._promote_selected(selected)

    def _promote_selected(self, selected: list[tuple[str, str]]) -> None:
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        for tier in ("hbm", "dram", "ssd"):
            metrics[f"{tier}_hit_rate"] = (
                metrics[f"{tier}_hit_tokens"] / query_tokens
                if query_tokens
                else 0.0
            )
