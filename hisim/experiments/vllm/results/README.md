...
- `rtx4090_longctx_fit_p*_o8_real.json`：512～8064 token、每长度 5 组 cold/warm 真机结果；
- `rtx4090_longctx_p*_o8_real.json`：每长度 3 组的先导实验，保留用于重复性审计；
- `rtx4090_tier_io_calibration*.json`：默认 NUMA 与 GPU-local NUMA 1 的 HBM/DRAM/SSD I/O；
- `rtx4090_qwen3_0.6b_tiered_kv_calibration_report.json`：重计算曲线、I/O 汇总、文件 hash 和
  leave-one-length-out 误差；
- `rtx4090_tiered_policy_engine_validation.json`：完整 vLLM Scheduler 的重算/加载切换验证。
